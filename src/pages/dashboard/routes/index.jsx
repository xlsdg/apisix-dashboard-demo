...
import { createStateSelector, createDispatchSelector } from '@/redux/actions/routes';
...
const Header = React.memo(props => {
  ...
  const left = (
    <Breadcrumb>
      ...
      <Breadcrumb.Item>{formatMessage({ id: 'dashboard.routes.menu' })}</Breadcrumb.Item>
    </Breadcrumb>
  );

  const handleClick = React.useCallback(() => history.push('/dashboard/routes/add'), []);
  const right = (
    <Button type="primary" onClick={handleClick}>
      <PlusOutlined />
      {` ${formatMessage({ id: 'dashboard.routes.new' })}`}
    </Button>
  );
